...
<%@include file="/WEB-INF/views/common/common_dialog.jsp"%>
 <s:form id="searchForm" action="depot" namespace="/" method="post">
    <div id="container">
...
--%>
                        <input type="button" value="新增" class="ui_input_btn01 btn_input"
                               data-url="<s:url namespace="/" action="depot_input"/>"/>
                        <input type="button" value="批量删除" class="ui_input_btn01 btn_batchDelete"
                               data-url="<s:url namespace="/" action="depot_batchDelete"/>"/>
                    </div>
                </div>
            </div>
        </div>
                ...
                <table class="table" cellspacing="0" cellpadding="0" width="100%" align="center" border="0">
                       ...
                       <%-- 定义一个变量,接收数据模型里面的表达式--%>
                            <%--h?keys: h是一个map结构的数据,代表获取到h里面的key的集合--%>
                                <%--遍历keys集合,把每次遍历的数据赋值给变量 key--%>
                                        <%--仓库名称:从map里面去去key对应的数据信息:中文名称--%>
                                    <th>仓库名称</th>
                                        <%--仓库地址:从map里面去去key对应的数据信息:中文名称--%>
                                    <th>仓库地址</th>
                        <th>操作</th>
                    </tr>
                    <tbody>
                    <s:iterator value="#result.listData">
                        <tr>
                            ...
                            <td>
                                <s:a namespace="" action="depot_input">
                                    <s:param name="depot.id" value="id"></s:param>
                                    编辑</s:a>
                                <s:url namespace="" action="depot_delete" var="deleteUrl">
                                    <s:param name="depot.id" value="id"></s:param>
                                </s:url>
